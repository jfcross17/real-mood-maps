...
def collect_historical_data_by_state(keywords, start_date, end_date, description):
    ...
    # Google Trends requires specific format for historical data
    # For short periods (< 36 hours), use 'YYYY-MM-DDTHH YYYY-MM-DDTHH'
    # For longer periods, use 'YYYY-MM-DD YYYY-MM-DD' (note the space)
    # But for 7 days, we need to calculate it differently
    
    from datetime import datetime as dt_parse
    start_dt = dt_parse.strptime(start_date, '%Y-%m-%d')
    end_dt = dt_parse.strptime(end_date, '%Y-%m-%d')
    
    # Calculate days difference
    days_diff = (end_dt - start_dt).days
    
    # For weekly data, Google Trends works better with relative timeframe
    # Use 'today X-Y' format where X is days ago
    days_ago_end = (datetime.now().date() - end_dt.date()).days
    days_ago_start = (datetime.now().date() - start_dt.date()).days
    
    # Google Trends format: 'today X-Y' where X > Y
    if days_ago_end < 0:
        # Date is in the future, can't collect
        print(f"   ⚠️ WARNING: Event date {end_date} is in the future!")
        timeframe = 'now 7-d'
    else:
        # Format: 'today X-Y' means "from X days ago to Y days ago"
        timeframe = f'today {days_ago_start}-{days_ago_end}'
